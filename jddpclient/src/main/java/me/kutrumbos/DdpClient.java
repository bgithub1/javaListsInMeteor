...
//		String errorMsg = "WebSocketClient error: "+ex.getMessage();
	...
	public Boolean syncConnect() throws InterruptedException {
		...
		getWsClient().connect();
		int tries = 0;
		while(getWsClient().getReadyState()!=WebSocket.READYSTATE.OPEN&&tries<10) {
	...
	public String call(String method, Object[] params){
		...
		if(send(callMsg)==READYSTATE.OPEN){
			return id;
		} else {
			return null;
		}
	 ...
	 */
	public READYSTATE subscribe(String name, Object[] params) {
	 ...
	 * @param msg
	 */
	public READYSTATE send(Map<DdpMessageField,Object> msg) {
		READYSTATE connState = getReadyState();
		if (connState == READYSTATE.OPEN) {
